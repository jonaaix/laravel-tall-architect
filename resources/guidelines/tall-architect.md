...
- Questions go at the end, under a `❓ Questions` marker. Numbered, one question per item; numbering continues across the conversation and never restarts at 1. Every question carries at least two lettered options (a, b, c) unless the answer is a value only the user has — a yes/no question works as options too (e.g. a) yes, b) no, c) look into it first). Option a) is the recommendation, prefixed `⭐ Recommended:`. "ok" means: recommendation accepted for every open question. Options, approaches and alternatives are lettered wherever else they appear too.
